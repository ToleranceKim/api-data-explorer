...
def main():
    ...
    if df_perf_detail is not None and not df_perf_detail.empty:
        df_perf_detail.to_csv("./data_detail/perf_detail_PF132236.csv", index=False, encoding="utf-8-sig")
        print("[공연 상세] PF132236 CSV 저장 완료")

    # 공연시설 상세
    df_venue_detail = collect_prfplc_detail("FC000001")
    if df_venue_detail is not None and not df_venue_detail.empty:
        df_venue_detail.to_csv("./data_detail/venue_detail_FC001247.csv", index=False, encoding="utf-8-sig")
